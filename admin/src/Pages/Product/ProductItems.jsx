...
export default function ProductItems() {
  ...
  const handleSearch = async(e) =>{
    if(!e.target.value) return handleFetchProduct();
    try {
      const response = await axios.get(`http://localhost:5200/product/search_product/${e.target.value}`);
      if (response.status !== 200) return alert('something went wrong');
      setProductData(response.data.data)
    } catch (error) {
      alert('something went wrong')
          ...
          <div className="w-full min-h-[610px]">
            <div className="max-w-[1220px] mx-auto py-5">
              <div className='flex items-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400'>
                ...
                <div className='w-[50%]'>
                  <input type="text" onChange={handleSearch} placeholder='Search Products'  className='p-1 w-full border-2 border-slate-400 rounded'/>
                </div>
              </div>  
